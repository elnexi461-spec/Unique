...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const color = roleColor[user.role] ?? "#60A5FA";
  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground">
      ...
      <button
        ...
        style={{ background: "rgba(8,18,50,0.9)", borderColor: "rgba(59,130,246,0.3)", color: "#60A5FA" }}
      ...
      <aside className={`fixed md:sticky top-0 left-0 h-screen w-64 flex flex-col z-40 transition-transform duration-300 border-r ${sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}
        style={{ background: "hsl(222 75% 5%)", borderColor: "rgba(59,130,246,0.12)" }}>

        {/* Brand */}
        <div className="p-5 border-b flex items-center gap-3" style={{ borderColor: "rgba(59,130,246,0.1)" }}>
          <Link href="/">
            <div className="flex items-center gap-2.5 cursor-pointer group">
              ...
              <div>
                ...
                <div className="text-[10px] font-mono uppercase tracking-widest" style={{ color: "rgba(96,165,250,0.6)" }}>
                  Institutional OS
                </div>
              </div>
            </div>
        ...
        <div className="mx-4 mt-3 px-3 py-2 rounded-lg flex items-center gap-2 border"
          ...
          <motion.div animate={{ opacity: [0.4, 1, 0.4] }} transition={{ duration: 2, repeat: Infinity }}
            className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: "#3B82F6" }} />
        ...
        <div className="p-4 border-t" style={{ borderColor: "rgba(59,130,246,0.1)" }}>
          ...
          <Button variant="outline" onClick={logout}
            ...
            style={{ borderColor: "rgba(59,130,246,0.15)", height: 34 }}>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        <div className="pointer-events-none fixed inset-0 opacity-[0.025] z-0"
          style={{ backgroundImage: "radial-gradient(circle at center, #3B82F6 1px, transparent 1px)", backgroundSize: "24px 24px" }} />

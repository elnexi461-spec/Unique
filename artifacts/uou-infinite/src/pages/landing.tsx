...
export default function LandingPage() {
  ...
  // Start fading only after 50px of scroll so the hero is fully visible on load.
  // Reverses automatically when scrolling back to the top.
  const heroY       = useTransform(scrollY, [50, 680], [0, -90]);
  const heroOpacity = useTransform(scrollY, [50, 560], [1, 0]);
      ...
      {phase === "landing" && (
        ...
        >
          ...
          <motion.section style={{ y: heroY, opacity: heroOpacity }}
            ...
            {/* ── Hero text block
                 ALL elements use `animate` (not whileInView) so they are
                 GUARANTEED visible on page load. The parent motion.section
                 already has style={{ opacity: heroOpacity }} which handles
                 the scroll-linked fade: fades out after 50px of scroll,
                 fades back in when scrolling back to the top. ── */}
            <div className="text-center max-w-5xl relative z-20">
              ...
              >
                <motion.img
                  src={import.meta.env.BASE_URL+"uou-logo.png"} alt="UOU"
              ...
              <motion.div
                ...
                transition={{ delay:0.15, duration:0.6, ease:EC }}
              ...
              {/* Headline — each word cascades in, z-20 to sit above background layers */}
                ...
                <h1 className="text-5xl md:text-7xl lg:text-8xl font-black tracking-tight leading-none">
                  {["Unique","Open","University"].map((w, wi) => (
                    <motion.span key={w}
                      ...
                      animate={{ opacity:1, y:0 }}
                      transition={{ delay:0.32+wi*0.18, duration:0.85, ease:EC }}
                      style={{
              ...
              <motion.p
                ...
                animate={{ opacity:1, y:0 }}
                transition={{ delay:0.72, duration:0.7, ease:EC }}
              ...
              <motion.div
                initial={{ opacity:0, y:24 }}
                animate={{ opacity:1, y:0 }}
                transition={{ delay:0.88, duration:0.6, ease:EC }}
                ...
                <Link href="/login">
                  <motion.div whileHover={{scale:1.04,y:-2}} whileTap={{scale:0.97}}>
                    <Button variant="outline" size="lg"
                      className="h-14 px-8 text-base font-semibold transition-all hover:bg-blue-500/10 hover:border-blue-400/60"
              ...
              <div className="flex items-center justify-center gap-8 flex-wrap">
                {STATS_HERO.map((s, i) => (
                  <motion.div key={s.label}
                    initial={{ opacity:0, y:24 }}
                    animate={{ opacity:1, y:0 }}
                    transition={{ delay:1.02+i*0.1, duration:0.6, ease:EC }}
